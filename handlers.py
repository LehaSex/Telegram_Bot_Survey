    ...
    if subject == "material":
        ...
        if action == "answers":
            ...
            if action == "add":
                ...
                db.db_connection.commit()
                db_cursor.close()
                await callback.message.edit_text("Вопрос добавлен!", reply_markup=admin_menu(check_superadmin(callback.from_user.id)))
                await state.clear()
            elif action == "incorrect":
                await callback.message.edit_text("Введите ответы (каждый с новой строки)")
                await state.set_state(AdminActions.admin_question_add_answers)
...
################ EDIT ##################
            ...
            action = callback.data.split("_")[3]
            if action == "confirm":
                await callback.message.edit_text("Выберите необходимое действие", reply_markup=admin_edit_question("admin_question_edit_text_input", "admin_question_edit_answers_input"))
            if action == "list":
                db_cursor = db.db_connection.cursor()
                db_cursor.execute("SELECT * FROM questions")
                questions = db_cursor.fetchall()
